...
3.  **Export to GeoPackage format** (ref: Section \@ref(readexport-vector-data))
    ...
    -   Save `sf_str_as` as a GeoPackage file named `sf_stream_as.gpkg` in the `data` folder.
        Use `sf::st_write()`.
...
4.  **Map streams and county boundaries** (ref: Section \@ref(mapping-vector-data))
    ...
    -   Create a map displaying both: North Carolina county boundaries from `sf_nc_county`, and Ashe County stream lines from `sf_str_as`.
        Use `ggplot2::ggplot()` with multiple `geom_sf()` layers.
...
1.  **Spatial join of survey sites and counties** (ref: Section \@ref(spatial-join))
    ...
    -   Load `sf_site_deq.rds` from the `data` folder using `readRDS()`.
...
3.  **Subset counties with more than ten sites** (ref: Section \@ref(vector-data-manipulation))
    ...
    -   From `sf_n_site`, retain only counties with more than three survey sites using `dplyr::filter()`.

    -   Assign the subsetted object to `sf_n3`.

4.  **Visualize the distribution on a stacked map** (ref: Section \@ref(mapping-vector-data))

    -   Create a layered (stacked) map showing both `sf_n_site` and `sf_n3`.
