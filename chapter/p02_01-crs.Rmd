...
1.  **Load the data** (ref: Section \@ref(getting-started))
    -   R has a built-in dataset `quakes`; Convert this data frame to `tibble` using `as_tibble()` function, and assign the resulting tibble to an object called `df_quakes`.
    -   Inspect `df_quakes` by printing and identify column names that contain longitude/latitude information.
2.  **Convert to an `sf` object** (ref: Section \@ref(getting-started))
    -   Convert `df_quakes` into an `sf` object using the `sf::st_as_sf()` function. Make sure to set the CRS to WGS84 (`EPSG:4326`) using the `crs` argument.
    -   Assign the resulting spatial object to `sf_quakes`.
    -   Visualize `sf_quakes` with `mapview::mapview()` function.
3.  **Calculate the distance between two points** (ref: Section \@ref(crs-transformation))
    -   Select the first two sites in `sf_quakes` and assign it to `sf_ft_quakes`.
    -   Transform the CRS of `sf_ft_quakes` to WGS 84 / UTM 17N using `sf::st_transfrom()`. Assign the resulting object to `sf_ft_quakes_proj`.
    -   Calculate the geographic distance between the two sites.
4.  **Export the spatial object**
    -   Save the `sf_quakes` object as an RDS file named `sf_quakes.rds` in the `data` folder. Use `saveRDS(sf_quakes, file = "data/sf_quakes.rds")`.
