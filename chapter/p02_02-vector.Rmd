...
3.  **Export to GeoPackage format** (ref: Section \@ref(readexport-vector-data))

    -   Save `sf_str_as` as a GeoPackage file named `sf_stream_as.gpkg` in the `data` folder. Use `sf::st_write()`.

4.  **Map streams and county boundaries** (ref: Section \@ref(mapping-vector-data))
...
5.  **Subset county layer to Ashe county and remap** (ref: Section \@ref(polygon) and Section \@ref(mapping-vector-data))
...
1.  **Spatial join of survey sites and counties** (ref: Section \@ref(spatial-join))
    ...
    -   In `sf_quakes_join`, earthquake events that occurred outside New Zealand should have `NA` in the `fid` column.
        Use `drop_na(sf_quakes, fid)` to remove quakes that occurred outside New Zealand, and assign the resulting object to `sf_quakes_nz`.
...
2.  **Count survey sites per county** (ref: [group operation](https://aterui.github.io/biostats/data-manipulation.html#group-operation))
    ...
    -   Using `sf_site_join`, calculate the number of survey sites in each county.
        Use `dplyr::group_by()` to group by `county`.
