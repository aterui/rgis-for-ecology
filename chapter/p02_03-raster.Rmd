...
Instead, we need to convert it to `stars` object using `stars::st_as_stars()` (see more instruction on `stars` package [here](https://tmieno2.github.io/R-as-GIS-for-Economists/stars-basics.html); this book does not cover this package).
...
Raster data represent spatial information as a grid of cells (pixels), where each cell holds a value.
These values can represent many types of data, but they generally fall into two broad categories: **continuous** and **discrete**.
...
Note that argument `y` must be supplied as a matrix or a dataframe.
...
You will get two values of elevation at these two locations as a dataframe.
...
In contrast, **discrete raster data** (also called categorical raster data) represent classes or categories with distinct boundaries.
Each cell stores an integer code that corresponds to a specific class, such as land cover type (e.g., forest, water, urban) or vegetation zone.
These values do not vary continuously and do not imply gradual transitions between categories.
Discrete rasters can be further categorized into **binary** and **code values with multiple categories**.
...
Unique values in this raster are confined to `0` and `1`, each representing non-forest and forest cells:
...
A benefit of binary representation is that it can be easily converted into summary information.
...
About 70% of the NC land is forest.
...
Apply `terra::unique()` to examine code values:

```{r}
unique(spr_land)
```

In this raster, cell values are **encoded as categorical codes**, for example:
...
Different raster datasets often use **different coding schemes**, depending on the source and purpose.
Therefore, it is essential to consult the accompanying **code table** or metadata to accurately interpret the categories represented in the raster.
...
As expected, this cell value contains "urban" category.
...
In this `cm` matrix, the code `1001` will be converted to `1`, while all other values will be converted to `0`.
...
Taking the mean of values from this raster yields the same value from the mean of `spr_for`:
...
4.  **Extract values** (ref: Section \@ref(raster-data-type))

    -   Read the fish sampling site data from `data/sf_finsync_nc.rds` and assign it to `sf_site`.

    -   `st_coordinates(sf_site)` will extract coordinates from the `sf` object as a dataframe.
        Assign this data frame to `df_xy`.

    -   Using `terra::extract()` function with inputs `spr_land` and `df_xy`, identify land use type at each sampling site.
        Assign the result to `df_land`.

    -   Identify the most common land use type at these sampling sites.

5.  **Reclassify** (ref: Section \@ref(raster-data-type))

    1.  Reclassify the values of `spr_land` to have binary entries **urban = 1** and **non-urban = 0**.
        After reclassification, assign the layer to `spr_urban`.

    2.  Calculate the proportion of urban land use in NC.
...
1.  **Merge raster files** (ref: Section \@ref(merge))
...
2.  **Crop raster to a defined extent** (ref: Section \@ref(crop))
    ...
    -   Plot `spr_tmp_camden` over `sf_nc_county` to confirm coverage.
        Use `ggplot2::ggplot()` with `geom_spatraster()` and `geom_sf()` (*Tip: Use `alpha = 0.25` in `geom_sf()` to make the polygons transparent.)*

3.  **Reproject raster and explore resampling** (ref: Section \@ref(reprojection))
